...
llm-store.ts     — LLM 설정 (localStorage 영속화, key: "mnemovault-llm-settings" v2).
...
interface LLMSettings {
  ...
  language: "en" | "ko";     // default: "en" — LLM 출력 언어
}
...
| 메서드 | 경로 | 요청 | 응답 |
|--------|------|------|------|
| POST | `/api/llm/ingest` | `{ fileName, content, fileType, llmConfig, language }` | `IngestLLMResult` |
| POST | `/api/llm/query` | `{ question, context, llmConfig, language }` | `{ answer: string }` |
...
### 즉시 수정이 필요한 것
1. ~~**`src/components/ingest/DropZone.tsx`** — 디버그 fetch 코드 제거~~ ✅ 완료
