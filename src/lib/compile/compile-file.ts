...
function buildSourcePage(
  ...
  const resolvedTitle = sourceTitle?.trim() || result.summary.title || rawFileName;
...
function generateUniqueSlug(
  ...
  const withSuffix = `${normalizedBase}-${defaultSuffix}`;
  if (!takenSlugs.has(withSuffix)) {
    return withSuffix;
  }

  let attempt = 2;
  while (takenSlugs.has(`${withSuffix}-${attempt}`)) {
    attempt += 1;
  }
...
function buildDebugPaths(fileName: string): {
  debugPayloadPath: string;
  requestFullPath: string;
  responseFullPath: string;
} {
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const basePath = `meta/llm-debug/${timestamp}-${sanitizeDebugName(fileName)}`;
  return {
    debugPayloadPath: `${basePath}-llm-request.json`,
    requestFullPath: `${basePath}-llm-request-full.json`,
    responseFullPath: `${basePath}-llm-response-full.txt`,
  };
}
...
export async function compileFile(
  ...
  try {
    ...
    if (logEnabled) {
      try {
        await writeFile(root, requestFullPath, JSON.stringify(requestBody, null, 2));
        await log("request", "file", "LLM request full body saved", requestFullPath, "request_saved");
      } catch (error) {
        await log(
          "error",
          "file",
          "Failed to save full LLM request body",
      ...
      for (const block of parsed.blocks) {
        ...
        if (message.event === "debug_payload") {
          ...
          try {
            ...
            await writeFile(root, debugPayloadPath, JSON.stringify(payload, null, 2));
            debugSaved = true;
            await log(
              "request",
              "file",
        ...
        if (message.event === "raw_chunk") {
          try {
            const payload = JSON.parse(message.data) as { text?: string };
            if (payload.text) {
              rawUpstreamResponse += `${payload.text}\n`;
            }
          } catch {
            continue;
          }
    ...
    if (logEnabled) {
      const responseRaw = rawUpstreamResponse || rawResponse;
      if (responseRaw) {
        try {
          await writeFile(root, responseFullPath, responseRaw);
          await log(
            "response",
            "file",
            "LLM response full body saved",
            responseFullPath,
            "response_saved"
          );
        } catch (error) {
          await log(
            "error",
            "file",
            "Failed to save full LLM response body",
            (error as Error).message,
    ...
    const takenSlugs = new Set<string>([
      ...existingSlugs,
      ...existingPagePaths.keys(),
    ]);

    // 3. Create/update source summary page
    const rawSourceTitle = llmResult.summary.title || file.fileName.replace(/\.[^.]+$/, "");
    let sourceSlug = toSlug(rawSourceTitle);
    let sourceTitle = rawSourceTitle;
    const sourcePathForSlug = existingPagePaths.get(sourceSlug);

    if (sourcePathForSlug && !sourcePathForSlug.startsWith("wiki/sources/")) {
      const previousSlug = sourceSlug;
      sourceSlug = generateUniqueSlug(sourceSlug, takenSlugs, "source");
      sourceTitle = `${rawSourceTitle} (Source)`;
      await log(
        "info",
        "file",
        "Detected cross-type slug collision for source page",
        `requested=${previousSlug}, resolved=${sourceSlug}`,
        "resolve_source_slug_collision"
      );
    }

    result.sourceSlug = sourceSlug;
    const sourcePath =
      existingPagePaths.get(sourceSlug) && existingPagePaths.get(sourceSlug)!.startsWith("wiki/sources/")
        ? existingPagePaths.get(sourceSlug)!
        : `wiki/sources/${sourceSlug}.md`;
    const sourceContent = buildSourcePage(llmResult, file.fileName, sourceTitle);
    ...
    existingSlugs.add(sourceSlug);
    existingPagePaths.set(sourceSlug, sourcePath);
    ...
    for (const concept of llmResult.concepts) {
      ...
      const existingPath = existingPagePaths.get(slug);
      if (existingPath) {
        const existing = await readFile(root, existingPath);
        const merged = mergeIntoExistingPage(
          existing,
        ...
        await writeFile(root, existingPath, merged);
        result.updatedSlugs.push(slug);
        await log("write", "file", `Merged concept: ${concept.name}`, existingPath, "merge_concept");
      } else {
    ...
    for (const entity of llmResult.entities) {
      ...
      if (existingPath) {
        ...
        await writeFile(root, existingPath, merged);
        result.updatedSlugs.push(slug);
        await log("write", "file", `Merged entity: ${entity.name}`, existingPath, "merge_entity");
      } else {
        const page = buildNewConceptOrEntityPage(
          entity.name,
          entity.content,
