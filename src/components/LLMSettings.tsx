...
export default function LLMSettings({ onClose }: LLMSettingsProps) {
  ...
  const [openRouterMissingModels, setOpenRouterMissingModels] = useState(false);
  ...
  const fetchOpenRouterModels = async () => {
    try {
      ...
      const enabled =
        typeof data.openrouterEnabled === "boolean" ? data.openrouterEnabled : true;
      const models = Array.isArray(data.models) ? data.models : [];
      const missingModels =
        typeof data.missingModels === "boolean" ? data.missingModels : false;

      setOpenRouterEnabled(enabled);
      setOpenRouterModels(models);
      setOpenRouterMissingModels(missingModels);
      ...
      setOpenRouterMissingModels(false);
      setOpenRouterModels(["openrouter/auto"]);
    }
  };
        ...
        <div className="mb-5">
          ...
          {!openRouterEnabled && (
            <p className="text-[10px] text-amber-300/80 mt-2">
              OpenRouter is disabled because one or more required settings are missing.
            </p>
          )}
          {!openRouterEnabled && openRouterMissingModels && (
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              {ollamaModels.length > 0 ? (
                ...
                <div className="w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-sm text-white/45">
                  No Ollama models found. Click Connect first.
                </div>
              )}
            </div>
