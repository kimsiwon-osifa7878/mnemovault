...
function isKoreanLocale(acceptLanguage: string | null): boolean {
  if (!acceptLanguage) {
    return false;
  }
  ...
  return acceptLanguage
    .split(",")
    .some((item) => item.trim().toLowerCase().startsWith("ko"));
}
...
export default async function LandingPage({
  ...
  const isKorean = isKoreanLocale(requestHeaders.get("accept-language"));

  const copy = isKorean
    ? {
        navGetStarted: "시작하기",
        heroTitlePrefix: "지식을",
        heroTitleHighlight: "컴파일",
        heroTitleSuffix: "하는 위키",
        heroDescriptionLine1: 'RAG의 "매번 재검색" 한계를 넘어,',
        heroDescriptionCompile: "증분 컴파일",
        heroDescriptionAccumulate: "영구적으로 축적",
        heroDescriptionTail: "하는 마크다운 위키 시스템",
        heroPrimaryCta: "위키 시작하기",
        heroSecondaryCta: "자세히 보기",
        operationsTitle: "4대 오퍼레이션",
        operationsDescription:
          "Karpathy가 정의한 LLM Wiki의 핵심 작업 흐름. 소스를 넣으면 위키가 복리로 성장합니다.",
        ingestDescription:
          "논문, 기사, 노트를 업로드하면 LLM이 분석하여 요약, 개념 페이지, 엔티티 페이지를 자동 생성합니다. 교차참조와 인덱스도 자동 갱신됩니다.",
        queryDescription:
          "위키에 질문하면 LLM이 index.md를 먼저 읽고 관련 페이지를 드릴다운하여 답변을 합성합니다. 좋은 답변은 위키에 재수록됩니다.",
        lintDescription:
          "주기적 헬스체크로 모순 감지, 낡은 주장 갱신, 고아 페이지 발견, 누락된 교차참조를 자동으로 찾아냅니다.",
        compileDescription:
          "Raw 소스에서 Wiki로의 변환 자체가 핵심 혁신. 단순 인덱싱이 아닌 구조화, 요약, 백링크 생성을 통한 지식 컴파일.",
        architectureTitle: "3-Layer 지식 구조",
        architectureDescription:
          "Karpathy의 원문에 기반한 3계층 아키텍처. Raw 소스는 불변으로 보존하고, LLM이 Wiki 레이어를 소유하며, Schema가 전체 구조를 정의합니다.",
        architectureCta: "직접 체험해보기",
        rawSourcesCaption: "불변 · LLM이 읽기만 함",
        wikiCaption: "LLM 소유 · 사람은 읽기/리뷰",
        schemaCaption: "사람 + LLM 공동 진화",
        schemaDescription: "폴더 구조, 컨벤션, 워크플로우 정의 (CLAUDE.md)",
        interfaceTitle: "3-Pane IDE 레이아웃",
        interfaceDescription:
          "사이드바에서 탐색하고, 에디터에서 편집하고, 채팅과 그래프로 지식을 확장합니다.",
        benefitsTitle: "왜 MnemoVault인가",
        benefit1Title: "복리로 성장하는 지식",
        benefit1Description:
          "소스를 추가할수록 교차참조가 자동으로 연결되고, 기존 페이지가 보강되며, 지식 네트워크가 풍부해집니다.",
        benefit2Title: "Context-Aware 채팅",
        benefit2Description:
          "현재 열린 문서와 이웃 노드를 자동으로 LLM 컨텍스트에 포함하여 더 정확한 답변을 생성합니다.",
        benefit3Title: "그래프 시각화",
        benefit3Description:
          "[[위키링크]] 관계를 Force-directed 그래프로 시각화하여 지식의 전체 구조를 한눈에 파악합니다.",
        footerTitle: "지식의 복리를 경험하세요",
        footerDescription:
          "첫 번째 소스를 넣는 순간부터 위키가 성장하기 시작합니다. Claude API 또는 로컬 Ollama로 바로 시작할 수 있습니다.",
        footerCta: "위키 시작하기",
      }
    : {
        navGetStarted: "Get Started",
        heroTitlePrefix: "A Wiki That",
        heroTitleHighlight: "Compiles",
        heroTitleSuffix: "Knowledge",
        heroDescriptionLine1: 'Beyond RAG\'s "retrieve every time" limitation,',
        heroDescriptionCompile: "incrementally compiles",
        heroDescriptionAccumulate: "permanently accumulates",
        heroDescriptionTail: "knowledge into a markdown wiki system",
        heroPrimaryCta: "Launch Wiki",
        heroSecondaryCta: "Learn More",
        operationsTitle: "Four Core Operations",
        operationsDescription:
          "The core LLM Wiki workflow defined by Karpathy. Add sources, and your wiki grows with compounding value.",
        ingestDescription:
          "Upload papers, articles, and notes. The LLM analyzes them and automatically creates summaries, concept pages, and entity pages while keeping cross-references and the index up to date.",
        queryDescription:
          "Ask questions to the wiki, and the LLM reads index.md first, then drills down into relevant pages to synthesize an answer. High-value answers are filed back into the wiki.",
        lintDescription:
          "Run periodic health checks to detect contradictions, refresh stale claims, discover orphan pages, and automatically add missing cross-references.",
        compileDescription:
          "The transformation from raw sources to wiki is the core innovation: not simple indexing, but structured knowledge compilation with summaries and backlinks.",
        architectureTitle: "3-Layer Knowledge Architecture",
        architectureDescription:
          "A three-layer architecture inspired by Karpathy's original pattern. Raw sources stay immutable, the LLM owns the wiki layer, and the schema defines the overall structure.",
        architectureCta: "Try It Live",
        rawSourcesCaption: "Immutable · Read-only for LLM",
        wikiCaption: "Owned by LLM · Humans read/review",
        schemaCaption: "Co-evolved by Humans + LLM",
        schemaDescription:
          "Defines folder structure, conventions, and workflow (CLAUDE.md)",
        interfaceTitle: "3-Pane IDE Layout",
        interfaceDescription:
          "Explore in the sidebar, edit in the editor, and expand knowledge through chat and graph views.",
        benefitsTitle: "Why MnemoVault",
        benefit1Title: "Compounding Knowledge",
        benefit1Description:
          "As you add more sources, cross-references are linked automatically, existing pages are enriched, and your knowledge network grows denser.",
        benefit2Title: "Context-Aware Chat",
        benefit2Description:
          "The current document and neighboring nodes are automatically included in the LLM context to generate more accurate answers.",
        benefit3Title: "Graph Visualization",
        benefit3Description:
          "Visualize [[wikilink]] relationships with a force-directed graph to understand the structure of your knowledge at a glance.",
        footerTitle: "Experience Knowledge Compounding",
        footerDescription:
          "Your wiki starts growing from the very first source you ingest. Start immediately with Claude API or local Ollama.",
        footerCta: "Launch Wiki",
      };
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-lg sm:text-xl text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">
            {copy.heroDescriptionLine1}
            <br className="hidden sm:block" />
            {isKorean ? "LLM이 지식을 " : "an LLM that "}
            <span className="text-white/70">{copy.heroDescriptionCompile}</span>
            {isKorean ? "하여 " : " and "}
            <span className="text-white/70">{copy.heroDescriptionAccumulate}</span>
            {isKorean ? "하는 " : " "}
            {copy.heroDescriptionTail}
          </p>
              ...
              <div className="p-5 rounded-xl border border-white/10 bg-white/[0.03]">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/30">{copy.rawSourcesCaption}</p>
                  </div>
                </div>
              ...
              <div className="p-5 rounded-xl border border-blue-500/20 bg-blue-500/[0.03] ring-1 ring-blue-500/10">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-400/50">{copy.wikiCaption}</p>
                  </div>
                </div>
                <p className="text-xs text-white/40 ml-11">
              ...
              <div className="p-5 rounded-xl border border-white/10 bg-white/[0.03]">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/30">{copy.schemaCaption}</p>
                  </div>
                </div>
                <p className="text-xs text-white/40 ml-11">
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="text-center p-6">
              ...
              <h3 className="font-semibold mb-2">{copy.benefit1Title}</h3>
              <p className="text-sm text-white/40 leading-relaxed">
                {copy.benefit1Description}
              </p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <h3 className="font-semibold mb-2">{copy.benefit2Title}</h3>
              <p className="text-sm text-white/40 leading-relaxed">
                {copy.benefit2Description}
              </p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <h3 className="font-semibold mb-2">{copy.benefit3Title}</h3>
              <p className="text-sm text-white/40 leading-relaxed">
                {copy.benefit3Description}
              </p>
            </div>
          </div>
        </div>
